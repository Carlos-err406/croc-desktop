...
Not yet verified on hardware: background survival and LAN / local-only mode.

### Background survival — built, not yet proven

There **is** a foreground service now (`TransferService`, generated by
`scripts/android-configure.mjs`; started and stopped from `src/android_fgs.rs`). It runs in
the same process as the activity and holds no transfer state — croc stays a child of the
app process, and the service exists only so Android doesn't freeze that process. Rust
starts it with the first transfer and stops it when the last one ends, so the notification
is only up while something is running.

Verified so far by build only: the Kotlin compiles, the `<service>` element survives
manifest merging with `android:foregroundServiceType="dataSync"`, and the crate still
cross-compiles. **What a device has to confirm:**

1. Start a receive, background the app, lock the phone — the transfer completes and the
   received file's SHA-256 matches.
2. The notification appears while transferring and disappears when done (including after a
   cancel, which kills croc and lets the reader thread stop the service).
3. Two concurrent transfers: the notification survives the first one finishing.
4. `adb shell dumpsys activity services dev.carlosd.crocdesktop` lists the service while a
   transfer runs and not after.

Then, and only then, drop the "keep the app open" caveat from the README.
