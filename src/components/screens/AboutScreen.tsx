        ...
        <div className="my-auto flex flex-col items-center gap-6 pt-6">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="max-w-[380px] text-[13px] leading-relaxed text-muted-foreground">
              ...
              <span className="font-medium text-foreground">croc</span> — send files and text to any
              device with a one-time code, encrypted end-to-end, peer-to-peer.
            </p>
          </div>
            ...
            <div className="flex w-full max-w-[360px] items-start gap-2.5 rounded-[12px] border border-warning-text/50 bg-warning-surface px-4 py-3 text-[12px] text-warning-text">
              ...
              <div>
                ...
                <div className="mt-0.5 leading-relaxed">
                  ...
                  <span className="break-all font-mono">{info.path ?? 'PATH'}</span>, not the
                  bundled {info.expectedVersion}. Transfers may fail with peers on the bundled
                  version. Remove or update the system croc so versions match.
                </div>
              </div>
            </div>
          )}
          ...
          updater.status === 'ready' ? (
            <div className="flex w-full max-w-[300px] items-center gap-2.5 rounded-[12px] border border-brand/40 bg-brand-surface px-4 py-2.5 text-[13px] text-brand-deep">
              ...
              {updater.status === 'downloading' ? (
                ...
                <>
                  <span className="flex-1">
                    Update{updater.version ? ` v${updater.version}` : ''} ready
                  </span>
          ...
          {!IS_MOBILE && (
            <div className="flex w-full max-w-[340px] items-center gap-3 rounded-[12px] border border-border bg-secondary/50 px-4 py-3">
              ...
              <Button
                ...
                variant="outline"
                className="shrink-0"
                onClick={() => croc.openUrl(ANDROID_URL)}
              >
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <Lock size={12} className="text-success-text" /> End-to-end encrypted · nothing stored
            in the cloud
          </div>
        </div>
      </div>
