...
const NON_CONNECTION =
  /too short|did ?n'?t match|declined|already exists|file error|no files|nothing to send/i;
...
export function ConnectionHint({
    ...
    return (
      <div className="w-full max-w-[440px] rounded-[12px] border border-border bg-secondary/50 px-4 py-3 text-left text-[12px] text-muted-foreground">
        ...
        <p className="mt-1 leading-relaxed">
          ...
          <span className="font-medium text-foreground">phone hotspots block this</span> (they
          isolate the devices connected to them).
        </p>
        <ul className="mt-1.5 list-disc space-y-0.5 pl-4">
          <li>
            Use a real <span className="font-medium text-foreground">Wi-Fi router</span> (a
            home/office network — it can even be one with no internet), not a phone hotspot.
          </li>
          <li>
            Make sure <span className="italic">both</span> devices have local-only mode on and are
            on the same Wi-Fi.
          </li>
  ...
  return (
    <div className="w-full max-w-[440px] rounded-[12px] border border-border bg-secondary/50 px-4 py-3 text-left text-[12px] text-muted-foreground">
      ...
      <p className="mt-1 leading-relaxed">
        A <span className="font-medium text-foreground">VPN</span> is the most common cause — it
        breaks croc's peer handshake even when the relay looks reachable. Restricted networks (some
        phone hotspots &amp; mobile carriers) can block it too.
      </p>
      <ul className="mt-1.5 list-disc space-y-0.5 pl-4">
        <li>
